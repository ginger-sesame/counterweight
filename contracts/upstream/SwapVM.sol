...
import { Context, ContextLib, VM, SwapRegisters, SwapQuery, ProtocolFee } from "@1inch/swap-vm/src/libs/VM.sol";
...
abstract contract SwapVM is EIP712, OnlyWethReceiver, Rescuable {
    ...
    function quote(
        ...
        _validateEntry(order, amount, takerTraitsAndData);
    ...
    function swap(
        ...
        bytes memory settlementSnapshot = _beforeSettlement(ctx);
        ...
        _afterSettlement(ctx, settlementSnapshot);
    ...
    // Counterweight extension hooks; transfer implementation above is unchanged.
    function _validateEntry(ISwapVM.Order calldata order, uint256 amount, bytes calldata data) internal view virtual;
    function _beforeSettlement(Context memory ctx) internal view virtual returns (bytes memory);
    function _afterSettlement(Context memory ctx, bytes memory snapshot) internal view virtual;
