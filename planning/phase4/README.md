...
Resolution: live width probes accept uint64/uint256 argument conditions but reject uint16/uint32 with otherwise identical policy and calldata. The two setTuning ABI arguments are now uint256; contract range validation remains intensity<=1000 and spread10..100 before casting to uint16 storage/events. This preserves the safety envelope and removes no policy checks. It changes the function selector and requires new epoch deployments plus F1/F2 regression; historical evidence remains tied to its original revision. See artifacts/p4-policy-width-diagnosis.json for diagnostic outcomes.

The full restricted policy now authorizes the uint256 (900,28) update and denies a valid second-controller request with policy_violation. The initial full O-matrix run then encountered a transient Uniswap Graph query timeout; actual permission/recovery coverage is still being completed. Full deterministic regression and pinned source/SDK checks pass after the ABI amendment, including 1,000 oversized argument cases that must reject before narrowing.

The permission/recovery diagnostic now completes all implemented cases, including live provider denials, pause, revocation, pending-signature rejection, resume, owner rotation, and unsafe settlement. It intentionally returns FAIL with synthetic Graph data. The live Graph relation timeout is resolved using [the documented primary-key/RPC identity contract](DATA_COMPATIBILITY.md); live preflight passes for both original sources. A full live F3 run and final coverage audit are in progress.
