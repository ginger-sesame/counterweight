...
The emergency policy is a separate single ALLOW rule for `pause()` to the same controller, same chain and zero value, no other methods. Owner policy/commissioning actions are separately reviewed; owner can sign deployment, approved token operations and controller administration. Runtime receives no signer credentials.
