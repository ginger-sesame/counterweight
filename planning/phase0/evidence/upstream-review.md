...
Additional targeted integration check: the initial accounting suite is explicitly non-Aqua, so it alone does not exercise shipping and Aqua settlement. Ran `forge test --match-path test/ControlsAqua.t.sol -vv` to resolve that gap. **6 passed, 0 failed, 0 skipped**; shipping, successful swap, expired-deadline rejection, balance-based access success/failure, and final taker balances are exercised through AquaSwapVMTest with mock tokens. Raw output is upstream-aqua-controls.txt. Total investigated upstream tests: **22 passed**. This still does not satisfy F1's canonical fork-token/custom-guard requirement.
