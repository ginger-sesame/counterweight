...
Status: IN PROGRESS. P5-A / E-01–E-03 and clean F1–F4 runtime proofs pass at `57e6e67`; independent-agent reproduction and final package acceptance remain pending. See [validation](evidence/validation.md).

## Deliverables and acceptance

- [x] E-01: `scripts/proofs/mvp.mjs` runs one continuous local fork history with actual Privy owner setup, 14 WETH/12,000 USDC allocation, two live completed-day Graph sources, restricted tuning, a 200 USDC fill, a 0.5 WETH guard rejection, and a valid forbidden Privy request. Retain exact state deltas, source/configuration/policy identities and receipts.
- [x] E-02: Inject stale data, recover from fresh observations, change inventory between quote and execution, force settlement rollback, pause/resume and revoke/re-enroll the updater. Record each expected failure and unchanged protected state. No implicit snapshot reset.
- [x] E-03: Persist a public transaction checkpoint before broadcast. A new process reconciles its receipt against chain/fork, contract code and immutable configuration identity without signing or broadcasting. Missing or uncertain transactions require reconciliation; no automatic replacement. Reject configuration drift, and report expired observation freshness separately from transaction completion.
- [x] Unit/component checkpoint tests: mined, pending, absent, transport failure, wrong chain, wrong fork, changed code/configuration, failed receipt and freshness expiry. Then exercise the actual subprocess against the live local fork.
...
- [x] Run deterministic suites, planning/source validation, F1/F2/F3 public CLI regression and F4 at the integrated runtime revision. Preserve failures; publish PASS only for completed assertions.
...
The shared technical contracts remain under tracked `planning/`. Local `docs/` and credentials remain unpublished under D14. Commands, architecture and [sponsor indexes](sponsors/README.md) are available; [validation](evidence/validation.md) records the clean results and remaining independent review.
...
The [sponsor indexes](sponsors/README.md) link concrete proof artifacts, not submitted bounty claims. [Validation](evidence/validation.md) identifies the runtime revision and audits. E-04 remains pending until an authorized independent agent completes [the reproduction assignment](INDEPENDENT_REPRODUCTION.md).
...
P5-A completion: 2026-09-09; agent `/root`; evidence [clean runtime validation](evidence/validation.md). P5-B remains open for E-04 and the final E-05 package audit.
