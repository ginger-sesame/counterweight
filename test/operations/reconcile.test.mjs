...
test('E-03 restart recognizes mined operation independently from observation expiry',async()=>{
 const chain=fixture();
 assert.deepEqual(await reconcileCheckpoint(chain,checkpoint,{now:1120}),{status:'CONFIRMED',observationFresh:true,action:'DO_NOT_REPEAT',transactionHash:hash,blockNumber:'11'});
 assert.equal((await reconcileCheckpoint(chain,checkpoint,{now:1121})).observationFresh,false);
 assert.equal((await reconcileCheckpoint(chain,checkpoint,{now:999})).observationFresh,false);
