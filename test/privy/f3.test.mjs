...
test('F3 CLI proves real Privy restrictions, live Graph settlement and operational recovery',{timeout:300000},async()=>{
 ...
 try{
  ...
  assert.equal(manifest.result,'PASS');assert.equal(manifest.phase4Status,'COMPLETE_MATRIX');assert.deepEqual(manifest.unprovenMethods,[]);assert.equal(manifest.batchDenialEnvironment.chainId,11155111);assert.deepEqual(manifest.batchDenialEnvironment.before,manifest.batchDenialEnvironment.after);assert.equal(manifest.chainId,31337);assert.equal(manifest.maker.toLowerCase(),manifest.deployer.toLowerCase());
  ...
  for(const label of ['updater wrong controller','updater approval widening','emergency cannot resume','revoked updater cannot sign','rotated-out administrator cannot complete quorum','policy retarget rejects previous controller','updater user operation signing','emergency user operation signing','updater provider broadcast method','emergency provider broadcast method','updater batched calls','emergency batched calls'])assert(ops.some(o=>o.label===label),label);
