...
function sample(index=0){
 ...
 return {envelope,source,context:{now:1788689100,head:{chainId:1,number:25917720,timestamp:1788689024},indexedBlock:{...meta.block},poolTokens:{[source.pool]:envelope.tokenMetadata},tokenIdentityBlock:25917720}};
}
...
test('G-03 exact source and fetch age, lag and RPC timestamp fallback',()=>{
 ...
 const lag=sample();lag.context.head.number=lag.context.indexedBlock.number+25;lag.context.tokenIdentityBlock=lag.context.head.number;assert.doesNotThrow(()=>run(lag));lag.context.head.number++;assert.throws(()=>run(lag));
