...
const requests={
 signUserOperation:{params:{chain_id:31337,contract:'0x69007702764179f14F51cdce752f4f775d74E139',user_operation:{sender:wallet.address,nonce:'0x0',call_data:encodeFunctionData({abi:parseAbi(['function execute(address dest,uint256 value,bytes func)']),functionName:'execute',args:[wallet.address,0n,'0x']}),call_gas_limit:'0x30d40',verification_gas_limit:'0x30d40',pre_verification_gas:'0x5208',max_fee_per_gas:'0xf4240',max_priority_fee_per_gas:'0xf4240',paymaster:'0x0000000000000000000000000000000000000000',paymaster_data:'0x',paymaster_verification_gas_limit:'0x0',paymaster_post_op_gas_limit:'0x0'}}},
 sendTransaction:{caip2:'eip155:31337',params:{transaction}},
 sendCalls:{caip2:'eip155:11155111',sponsor:false,params:{calls:[{to:wallet.address,value:'0x0',data:'0x'}]}}
};
const batchRpc=createPublicClient({transport:http('https://ethereum-sepolia-rpc.publicnode.com',{timeout:10000,retryCount:0})});
const batchState=async()=>({chainId:await batchRpc.getChainId(),balance:String(await batchRpc.getBalance({address:wallet.address})),nonce:await batchRpc.getTransactionCount({address:wallet.address,blockTag:'pending'})});
const before=await batchState();assert.equal(before.chainId,11155111);assert.equal(before.balance,'0');
...
const after=await batchState();assert.deepEqual(after,before,'testnet balance and nonce unchanged');
const result=results.every(r=>r.result==='POLICY_DENIED')?'PASS':'UNPROVEN';
await writeFile(output,JSON.stringify({runAt:new Date().toISOString(),result,walletId:wallet.id,chainId:31337,batchEnvironment:{before,after},limitations:['No local RPC state assertion; this diagnostic does not replace the integrated F3 full matrix.','No policy changes; zero-value unsponsored testnet batch requests must be denied without public-network effects.'],results},null,2)+'\n',{flag:'wx'});
