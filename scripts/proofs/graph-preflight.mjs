...
try {
 ...
 if(collection.result!=='COLLECTED') throw new Error('two-source collection unavailable');
 ...
 if(created) await writeFile(`${out}/failure.json`,JSON.stringify({result:'FAIL',reason},null,2));
 console.error(JSON.stringify({result:'FAIL',reason}));process.exitCode=1;
}
